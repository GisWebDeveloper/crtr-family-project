...
@Component({
    selector: 'app-need-actions',
    templateUrl: './need-actions.component.html',
    styleUrls: ['./need-actions.component.scss']
})
export class NeedActionsComponent implements OnInit {

    public static readonly COUNT_STAT_ID_FORM_EXCEPTION = 1056;
    ...
    initCountStatList() {
        this.workspaceService.getStat({countCode: 'S05', regionId: this.params.filterRegionId}).subscribe({
            next: response => {
        ...
        if (ci && ci.countDictionary.code.startsWith('S')) {
            ...
            this.workspaceService.getStatPage(filterRequest).subscribe({
                next: response => {
                    ...
                    this.disableForm = ci.countDictionary.id == NeedActionsComponent.COUNT_STAT_ID_FORM_EXCEPTION;
                },
                error: errorResponse => {
                    this.utilService.notifyError(this.utilService.getErrorMessage(errorResponse));
                }
