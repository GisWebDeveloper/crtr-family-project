...
@Component({
  selector: 'app-gov-agency-mior-report-dynamic-neet',
  templateUrl: './gov-agency-mior-report-dynamic-neet.component.html',
  styleUrls: ['./gov-agency-mior-report-dynamic-neet.component.scss']
})
export class GovAgencyMiorReportDynamicNeetComponent implements OnInit {
    ...
    reportList: Array<ReportDynamicMiorNeet> = [];
    reportTotal: ReportDynamicMiorNeet = new ReportDynamicMiorNeet();
    listCntId: number[] = [];
    ...
    initReport() {
        ...
        this.reportService.getReportDynamicMiorNeet(request).subscribe({
            next: response => {
    ...
    calcTotals(list: Array<ReportDynamicMiorNeet>) {
        let total = new ReportDynamicMiorNeet();
        ...
        total.cnt24 = total.cnt23 / total.cnt3 * 100;
        total.cnt25 = this.utilService.getArrayFieldSum(list, 'cnt25');
        total.cnt26 = total.cnt25 / total.cnt3 * 100;
    ...
    showListModal(cntId: number, region?: Region) {
        if (this.reportCategory) {
            this.eventShowListModal.emit({
                ...
                region: region,
                cntId: cntId,
                cntName: this.utilService.getTranslationValue('gov-agency-page.dynamic.mior.neet.cnt-' + (cntId - 300))
            });
        }
    }
