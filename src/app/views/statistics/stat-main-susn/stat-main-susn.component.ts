...
@Component({
    selector: 'app-stat-main-susn',
    templateUrl: './stat-main-susn.component.html',
    styleUrls: ['./stat-main-susn.component.scss']
})
export class StatMainSusnComponent implements OnInit {

    @Input() statSusnDictionary: Dictionary[];
    @Output() eventShowListModal: EventEmitter<any> = new EventEmitter<StatItemDescription>();

    statItemList: Array<StatItemSUSN> = [];
    regionStatItemList: Array<StatItemSUSN> = [];
    regionChildStatItemList: Array<StatItemSUSN> = [];
    ...
    ngOnInit(): void {
        this.statService.getStatSUSN().subscribe({
            next: response => {
    ...
    calculateTotal() {
        let totalStat = new StatItemSUSN();
    ...
    selectStatRegion(statItem: StatItemSUSN) {
    ...
    showListModal(statItem: StatItemSUSN, dictionary: Dictionary) {
        // @ts-ignore - SET PROTO MANUALLY TO USE instanceof
        statItem.__proto__ = StatItemSUSN.prototype;

        let statDescription = new StatItemDescription();
        statDescription.id = dictionary.id;
        statDescription.type = 'SUSN';
        statDescription.statItem = statItem;
        statDescription.table = this.utilService.getTranslationValue('stat-page.labels.susn-desc');
