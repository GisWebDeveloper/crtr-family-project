...
@Component({
  selector: 'app-form-x-category',
  templateUrl: './form-x-category.component.html',
  styleUrls: ['./form-x-category.component.scss']
})
export class FormXCategoryComponent implements OnInit {

    @Input() hasAdminPermissions: boolean;
    @Input() eventShowFormXCategory: EventEmitter<string>;
    ...
    ngOnInit(): void {
        ...
        this.eventShowFormXCategory.asObservable().subscribe(iinValue => {
            this.iin = iinValue;
    ...
    private initDictionaries() {
        this.dictionaryService.getDictionaryGovAction(GovAgencyService.X_CATEGORY_DICTIONARY_CODE_PREFIX).subscribe({
            next: response => {
    ...
    private initPersonCountActions(iin: string) {
        ...
        request.iin = iin;
        request.countCode = GovAgencyService.X_CATEGORY_CODE_PREFIX;
