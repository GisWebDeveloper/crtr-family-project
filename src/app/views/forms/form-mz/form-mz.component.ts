...
@Component({
    selector: 'app-form-mz',
    templateUrl: './form-mz.component.html',
    styleUrls: ['./form-mz.component.scss']
})
export class FormMzComponent implements OnInit {

    @Input() hasAdminPermissions: boolean;
    @Input() eventShowFormMZ: EventEmitter<string>;
    ...
    ngOnInit(): void {
        ...
        this.eventShowFormMZ.asObservable().subscribe(iinValue => {
            this.iin = iinValue;
    ...
    private initPersonCountActions(iin: string) {
        ...
        request.iin = iin;
        request.countCode = GovAgencyService.MZ_LIST_CODE_PREFIX;
    ...
    getCountDictionaryName(countCode: string, actionCode?: string) {
        ...
        if (actionCode) {
            ...
            if (countCode == GovAgencyService.MZ_LIST_OSMS_CODE) dictionaryArray = this.dictionaryActionOSMS;
